...
fn main() {
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            let autolaunch_enabled = app.autolaunch().is_enabled().unwrap_or(false);

            let autolanch_i = CheckMenuItem::with_id(
                app,
                "autolaunch",
                "Autolaunch",
                true,
            ...
            let menu = Menu::with_items(app, &[&autolanch_i, &quit_i])?;
            ...
            let tray = TrayIconBuilder::new()
                ...
                .on_menu_event(move |app, event| match event.id.as_ref() {
                    ...
                    "autolaunch" => {
                        info!("autolaunch menu item was clicked");
                        let autolaunch = app.autolaunch();
                        let autolaunch_i = menu.get("autolaunch").unwrap();
                        let autolaunch_i = autolaunch_i.as_check_menuitem().unwrap();
                        if autolaunch.is_enabled().unwrap() {
                            autolaunch.disable().unwrap();
                            autolaunch_i.set_checked(false).unwrap();
                        } else {
                            autolaunch.enable().unwrap();
                            autolaunch_i.set_checked(true).unwrap();
                        }
                    }
                    _ => {
